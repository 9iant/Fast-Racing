...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  poseROS.header.frame_id = string("world_enu");
  ...
  velROS.header.frame_id = string("world_enu");
  ...
  if (cov_pos)
  {
    ...
    covROS.header.frame_id = string("world_enu");
  ...
  if (cov_vel)
  {
    ...
    covVelROS.header.frame_id = string("world_enu");
  ...
  if ((t - pt).toSec() > 0.5)
  {
    trajROS.header.frame_id = string("world_enu");
  ...
  // Sensor availability
  sensorROS.header.frame_id = string("world_enu");
  ...
  if (1)
  {
    ...
    broadcaster->sendTransform(tf::StampedTransform(transform,   msg->header.stamp, string("world_enu"),  string("base")));      
...
int main(int argc, char** argv)
{
  ...
  n.param("frame_id",   _frame_id, string("world_enu") );    
